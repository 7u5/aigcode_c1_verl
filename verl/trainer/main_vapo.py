# verl/trainer/main_vapo.py
import torch
from verl.algorithms.vapo import VAPO
...
def parse_args():
    ...
    parser.add_argument("--beta", type=float, default=0.1, help="Temperature parameter")
    return parser.parse_args()

def main():
    args = parse_args()
    initialize_megatron()  # 初始化 Megatron-LM 环境
    ...
    # 初始化 VAPO 和工作进程
    vapo = VAPO(model, reward_model, value_model, beta=args.beta)
    workers = [FSDPWorker(model, rank) for rank in range(torch.distributed.get_world_size())]

    # 训练
    for step in range(args.max_steps):
        batch = next(data_loader)
        loss = vapo.train_step(batch)
        workers[0].reshard("training")  # 动态重新分片
        if step % 10 == 0:
