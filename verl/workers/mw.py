...
class SGLangRollout:
    """
    SGLang-based rollout engine for sequence generation.
    """
    def __init__(self, model_path=None, actor_module=None, config=None, tokenizer=None, model_hf_config=None, device_mesh=None, trust_remote_code=False):
        from sglang import Runtime, set_default_backend
        from sglang.backend.runtime import Runtime as SGLangRuntime
        ...
        tensor_parallel_size = config.tensor_model_parallel_size if device_mesh is None else device_mesh.size(dim="infer_tp")
        self.runtime = Runtime(
            model_path=model_path if model_path else config.model_path,
            tokenizer_path=config.tokenizer_path if hasattr(config, 'tokenizer_path') else None,
        ...
        # Store inference engine (SGLang runtime)
        self.inference_engine = self.runtime
        log_gpu_memory_usage("After building SGLang rollout", logger=logger)

    def generate_sequences(self, prompts: DataProto):
        """
        Generate sequences using SGLang.
        """
        from sglang import function, system, user, assistant

        # Convert prompts to SGLang format
        input_ids = prompts.batch["input_ids"].cpu().numpy()
        batch_size = input_ids.shape[0]
        prompts_text = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in input_ids]
        ...
        def generate(prompt):
            ...
            user(prompt)
            assistant(max_tokens=self.config.max_response_length, temperature=self.config.temperature)  # Adjust based on config

        # Run generation
        outputs = []
        for prompt in prompts_text:
            result = generate.run(prompt=prompt)
            outputs.append(result["text"])

        # Convert outputs to DataProto
        output_ids = [self.tokenizer.encode(text, add_special_tokens=False) for text in outputs]
        max_length = max(len(ids) for ids in output_ids)
        padded_ids = [ids + [self.tokenizer.pad_token_id] * (max_length - len(ids)) for ids in output_ids]
        output_tensor = torch.tensor(padded_ids, dtype=torch.long)

        output = DataProto(
            batch={"generated_ids": output_tensor},
...
class ActorRolloutRefWorker(MegatronWorker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        print("init_workers - ref_init-L 84: torch.distributed initialized:", torch.distributed.is_initialized())
        ...
        print("init_workers - ref_init done-L 102: torch.distributed initialized:", torch.distributed.is_initialized())
    ...
    def _build_rollout(self, trust_remote_code=False):
        if self.config.rollout.name == "vllm":
            ...
            log_gpu_memory_usage("After building SGLang sharding manager", logger=logger)
        else:
            raise NotImplementedError("Only vllm and sglang rollouts are supported with Megatron now")
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            ...
            print("init_workers - init_model-L 316: torch.distributed initialized:", torch.distributed.is_initialized())
        ...
        print("init_workers - is_ref-L 332: torch.distributed initialized:", torch.distributed.is_initialized())
        if self._is_actor:
    ...
    def set_model_parameters(self, params):
        if self._is_actor and self.actor_module is not None:
            model = self.actor_module[0] if isinstance(self.actor_module, list) else self.actor_module
            state_dict = model.state_dict()
            param_keys = list(state_dict.keys())
            if len(params) != len(param_keys):
                raise ValueError(f"Parameter count mismatch: expected {len(param_keys)}, got {len(params)}")
            new_state_dict = {k: v for k, v in zip(param_keys, params)}
            model.load_state_dict(new_state_dict)
            model.cuda()
            return True
        elif self._is_ref and self.ref_module is not None:
            model = self.ref_module[0] if isinstance(self.ref_module, list) else self.ref_module
            state_dict = model.state_dict()
            param_keys = list(state_dict.keys())
            if len(params) != len(param_keys):
                raise ValueError(f"Parameter count mismatch: expected {len(param_keys)}, got {len(params)}")
            ...
            raise RuntimeError("No model available to set parameters (neither actor nor ref)")
    ...
    def get_model_parameters(self):
        if self._is_actor and self.actor_module is not None:
            ...
            raise RuntimeError("No model available to get parameters (neither actor nor ref)")
    ...
    def update_actor(self, data: DataProto):
        ...
        estimated_flops, promised_flops = self.flops_counter.estimate_flops as_flops_per_second(total_tokens, elapsed_time, batch_size) = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
    ...
    def save_checkpoint(self, checkpoint_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        self.checkpoint_mananger.save_checkpoint(
